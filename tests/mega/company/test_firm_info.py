...
class TestFirmInfo(BaseTestCase):

    def test_firm_info(self):
        """获取企业顶部信息"""
        resp = self.client.firm_info(id=D.firm_info.id)
        self.assertEqual(resp.code, 10000)

    def test_firm_info1(self):
        """获取企业顶部信息"""
        resp = self.client.firm_info(id=D.firm_info.id1)
